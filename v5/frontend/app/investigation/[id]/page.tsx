...
import {
  getInvestigation,
  getInvestigationEvaluation,
  getInvestigationReport,
  type Investigation,
  type InvestigationEvaluation,
  type Report,
} from "@/lib/api";
import { InvestigationQualityPanel } from "@/components/investigation/investigation-quality-panel";
...
export default function InvestigationWorkspaceDetailPage() {
    ...
    try {
      ...
      if (data.report_id && data.status === "completed") {
        ...
        try {
          ...
          setEvaluation(null);
          setEvalError(
            e instanceof Error ? e.message : "Evaluation not available"
          );
        } finally {
  ...
  return (
    <div
      className="flex h-[calc(100vh)] flex-col bg-surface-muted"
      data-testid="page-investigation-workspace"
    >
      <header className="border-b border-surface-border bg-white px-6 py-4 shrink-0">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {inv.status === "completed" && inv.report_id && (
              <Link href={`/replay/${id}`}>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <div className="flex flex-1 min-h-0 overflow-hidden">
            ...
            <aside
              className="w-96 shrink-0 border-l border-surface-border bg-white p-4 overflow-y-auto block"
              data-testid="workspace-charts-panel"
            >
