"""V5 deterministic investigation orchestration — full analytics pipeline."""
...
from workflow.workflow_rules import WorkflowRuleConfig, load_workflow_rule_config
...
class InvestigationWorkflow:
    ...
    def __init__(
        ...
        *,
        ...
        self._router = router or WorkflowRouter(
            rule_config, config_root=config_root
        )
        self._rule_config = rule_config or load_workflow_rule_config(config_root)
    ...
    def _persist_findings(self) -> None:
        """Write runtime findings to risk intelligence storage (non-blocking)."""
        assert self._ctx is not None
        if not self._ctx.findings:
            return
        try:
            from intelligence_memory.storage.finding_storage_service import (
                FindingStorageService,
            )

            needs: bool | None = None
            if self._ctx.monitor_finding is not None:
                needs = bool(self._ctx.monitor_finding.needs_investigation)
            FindingStorageService().persist_investigation_findings(
                self._ctx.investigation_id,
                self._ctx.metric_name,
                self._ctx.findings,
                analysis_date=self._ctx.analysis_date,
                workflow_status=self._ctx.workflow_status.value,
                needs_investigation=needs,
                trigger_reason=self._ctx.trigger_reason or None,
            )
        except Exception as exc:
            logger.warning("Findings persistence failed: %s", exc, exc_info=True)
